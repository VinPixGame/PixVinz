document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- 7. GAME PAGE PREVIEW BUTTON LOGIC (10s POPUP FOR 5 COINS) ---
  let previewTimer = null;
  const previewBtn = document.getElementById('previewBtn'); // Make sure your preview button has this ID on the game page
  ...
  if (previewBtn) {
    previewBtn.addEventListener('click', () => {
      ...
      if (modalTitle) modalTitle.innerText = `LEVEL ${parseInt(levelNum).toString().padStart(2, '0')} PREVIEW`;
      if (modalImg) modalImg.src = `image/level${levelNum}.jpeg`;
      if (modal) modal.classList.remove('hidden');
