...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function startGateCycle() {
    ...
    function cycle() {
      ...
      gateTimer = setTimeout(() => {
        setGateState(false);
        gateTimer = setTimeout(cycle, 1500);
      }, 1100);
    }
    gateTimer = setTimeout(cycle, 1500);
  }
  ...
  const spendingKey = `coincraze_spending_${coinKey}`;
  let spendingProgress = parseFloat(localStorage.getItem(spendingKey)) || 0;
  ...
  if (savedItemsJson) {
    try {
      ...
      if (Array.isArray(parsed) && parsed.length) {
        parsed.forEach(data => {
          const deck = data.deck === 'upper' ? 'upper' : 'lower';
          const height = playArea.clientHeight;
          let y = Number(data.y) || 20;

          if (deck === 'upper') {
            y = Math.min(y, height * (upperTop + upperHeight) - NORMAL_COIN_RADIUS);
          } else {
            y = Math.min(y, height * (lowerTop + lowerHeight) - NORMAL_COIN_RADIUS);
          }
  ...
  function initPreloadedItems() {
    const width = playArea.clientWidth;
    const height = playArea.clientHeight;
    const lowerFloorY = height * (lowerTop + lowerHeight) - 14;
    ...
    for (let i = 0; i < 24; i++) {
      ...
      for (let attempt = 0; attempt < 150; attempt++) {
        ...
        y = height * 0.57 + Math.random() * height * 0.22;
        valid = true;

        for (const other of activeItems) {
          if (other.deck !== 'lower') continue;
          const dx = x - other.x;
          const dy = y - other.y;
          const d = Math.sqrt(dx * dx + dy * dy);
          if (d < (radius + other.radius) * 0.78) {
            valid = false;
            break;
          }
        }
      ...
      spawnItem(x, y, 'coin', 5, 'medium', false, 'resting', 'lower');
    }
    saveGameState();
  }
  ...
  function addShakeEnergy(amount) {
    ...
    if (shakeBarFill) shakeBarFill.style.width = `${shakeEnergy}%`;
    if (shakeBtn) {
  ...
  function updatePusher(now) {
    let dt = (now - lastTime) / 16.6666667;
    lastTime = now;
    if (!isFinite(dt)) dt = 1;
    dt = Math.min(dt, 1.5);

    pusherZ += PUSHER_SPEED * pusherDirection * dt;
    ...
    for (let s = 0; s < subSteps; s++) {
      updatePhysics(dt / subSteps, playArea.clientWidth, playArea.clientHeight);
    }
  ...
  /* =========================================================
     PHYSICS & PLINKO REBOUNDS
     ========================================================= */

  function updatePhysics(dt, width, height) {
    const upperFloor = height * (upperTop + upperHeight) - 14;
    const lowerFloor = height * (lowerTop + lowerHeight) - 14;
    const upperFront = upperFloor;
    const lowerFront = lowerFloor;
    const exitCenter = width / 2;
    const exitHalf = EXIT_WIDTH / 2;

    const pegs = [
      { x: width * 0.3, y: height * 0.22, r: 8 },
      { x: width * 0.5, y: height * 0.25, r: 8 },
      { x: width * 0.7, y: height * 0.22, r: 8 },
      { x: width * 0.4, y: height * 0.30, r: 8 },
      { x: width * 0.6, y: height * 0.30, r: 8 }
    ];

    for (const item of activeItems) {
      if (item.state !== 'falling' && item.state !== 'transition') {
        continue;
      }

      item.vy += 0.9 * dt;
      if (item.vy > 15) item.vy = 15;

      item.x += item.vx * dt;
      item.y += item.vy * dt;

      for (const peg of pegs) {
        const dx = item.x - peg.x;
        const dy = item.y - peg.y;
        const dist = Math.sqrt(dx * dx + dy * dy);
        if (dist < item.radius + peg.r) {
          const nx = dx / dist;
          const ny = dy / dist;
          item.x = peg.x + nx * (item.radius + peg.r);
          item.y = peg.y + ny * (item.radius + peg.r);
          item.vx = nx * (Math.abs(item.vx) + 3) + (Math.random() - 0.5) * 4;
          item.vy = -Math.abs(item.vy) * 0.4 + 1;
        }
      }

      const leftWall = item.radius + 2;
      const rightWall = width - item.radius - 2;
      ...
      if (item.deck === 'upper') {
        if (item.y + item.radius >= upperFloor) {
          item.y = upperFloor - item.radius;
          item.vy = 0;
          item.vx = 0;
          item.state = 'resting';
        }
      } else if (item.deck === 'transition') {
        if (item.y + item.radius >= lowerFloor) {
          item.y = lowerFloor - item.radius;
          item.vy = 0;
          item.vx = 0;
          item.state = 'resting';
          item.deck = 'lower';
    ...
    const pusherTop = height * upperTop + pusherZ;
    const pusherHeight = Math.max(24, Math.min(34, height * 0.055));
    const pusherBottom = pusherTop + pusherHeight;

    for (const item of activeItems) {
      if (item.state !== 'resting' || item.deck !== 'upper') continue;
      ...
      if (pusherDirection > 0) {
        const coinFront = item.y - item.radius;
        if (coinFront <= pusherBottom + 2 && item.y + item.radius >= pusherTop) {
          const pushAmount = PUSHER_SPEED * dt * 2.4;
          item.y += pushAmount;
          item.vy = pushAmount / Math.max(dt, 0.001);

          if (item.y + item.radius >= upperFront) {
            item.y = upperFront - item.radius + 4;
            item.state = 'transition';
            item.deck = 'transition';
            item.vy = Math.max(2.5, item.vy);
            item.vx *= 0.15;
          }
        }
      }
    ...
    resolveDeckCollisions(
      activeItems.filter(item => item.state === 'resting' && item.deck === 'upper'),
      upperFloor,
      width
    );

    const lowerItems = activeItems.filter(item => item.state === 'resting' && item.deck === 'lower');

    for (const item of lowerItems) {
      item.vx *= Math.pow(0.82, dt);
      item.vy = 0;
      if (Math.abs(item.vx) < 0.003) item.vx = 0;
      item.x += item.vx * dt;

      if (item.x < item.radius + 2) {
        item.x = item.radius + 2;
        item.vx = 0;
      }
      if (item.x > width - item.radius - 2) {
        item.x = width - item.radius - 2;
        item.vx = 0;
      }
      if (item.y > lowerFront - item.radius) {
        item.y = lowerFront - item.radius;
        item.vy = 0;
      }
      renderItem(item);
    }

    resolveDeckCollisions(lowerItems, lowerFront, width);

    if (pusherDirection > 0 && pusherZ > MAX_PUSH_DISTANCE * 0.72) {
      const transferStrength = PUSHER_SPEED * dt * 0.9;
      const lowerPushZone = lowerTop * height;
      for (const item of lowerItems) {
        if (item.y > lowerPushZone) {
          item.vx += (exitCenter - item.x) * transferStrength * 0.012;
        }
      }
    }
    ...
    for (let i = activeItems.length - 1; i >= 0; i--) {
      ...
      if (item.state !== 'resting' || item.deck !== 'lower') continue;

      const distanceFromCenter = Math.abs(item.x - exitCenter);
      const insideHole = distanceFromCenter < exitHalf - item.radius * 0.25;
      const atHoleEdge = item.y + item.radius >= height - EXIT_WIDTH * 0.5;

      if (insideHole && atHoleEdge && gateOpen) {
        triggerPayout(item);
      }
    }
  }

  function triggerPayout(item) {
    playSound('win');
    coinCount += item.value;
    updateBalanceDisplay();
  ...
  function resolveDeckCollisions(items, floor, width) {
    ...
    for (let pass = 0; pass < 4; pass++) {
      for (let i = 0; i < items.length; i++) {
        ...
        for (let j = i + 1; j < items.length; j++) {
          ...
          const physicalA = a.radius * collisionScale;
          const physicalB = b.radius * collisionScale;
          const minimum = physicalA + physicalB;
          ...
          if (relativeVx !== 0) {
            const transfer = relativeVx * 0.08;
            a.vx += transfer;
            b.vx -= transfer;
          }
        }
      ...
      for (const item of items) {
        if (item.y + item.radius > floor) {
          item.y = floor - item.radius;
          item.vy = 0;
        }
        if (item.x < item.radius + 2) {
          item.x = item.radius + 2;
  ...
  function renderItem(item) {
    ...
    item.element.style.left = `${item.x - item.radius}px`;
    item.element.style.top = `${item.y - item.radius}px`;
    item.element.style.width = `${item.size}px`;
    item.element.style.height = `${item.size}px`;
    item.element.style.zIndex = item.type === 'xp' ? '40' : '30';
  }
  ...
  if (dropCoinBtn) {
    dropCoinBtn.addEventListener('click', () => {
      ...
      localStorage.setItem(spendingKey, spendingProgress);
      addShakeEnergy(6);

      const width = playArea.clientWidth;
      const randomX = width / 2 + (Math.random() - 0.5) * 30;
  ...
  const XP_TIMER_KEY = `coincraze_last_xp_${coinKey}`;

  function checkXPTimer() {
    const now = Date.now();
    const lastXP = parseInt(localStorage.getItem(XP_TIMER_KEY)) || now;

    if (!localStorage.getItem(XP_TIMER_KEY)) {
      localStorage.setItem(XP_TIMER_KEY, now.toString());
      return;
    }
  ...
  checkXPTimer();
  setInterval(checkXPTimer, 5000);

  function spawnBonusXP() {
    const width = playArea.clientWidth;
    const randomX = XP_RADIUS + Math.random() * Math.max(1, width - XP_RADIUS * 2);
    spawnItem(randomX, XP_RADIUS + 2, 'xp', XP_REWARD, 'xp', true, 'falling', 'upper');
    playSound('win');
  }
  ...
  function spawnItem(startX, startY, type, value, sizeClass, isFalling, initialState, deck) {
    ...
    if (type === 'xp') {
      size = XP_SIZE;
      radius = XP_RADIUS;
      itemEl.className = 'pusher-item xp-bonus';
      const xpImg = document.createElement('img');
      xpImg.src = 'image/xp.png';
      xpImg.alt = '';
      itemEl.appendChild(xpImg);
    } else {
      size = NORMAL_COIN_SIZE;
      radius = NORMAL_COIN_RADIUS;
      itemEl.className = 'pusher-item coin-item';
